...
module.exports.run = async (client, message, args) => client.error(message, 'The Hypixel API has currently broken because of JSON-schema \:(');
    ...
    try {
        if (!args.length)message.reply({embeds: [new MessageEmbed().setColor('RED').setDescription('You need to include a player\'s name!')]});
            ...
            if (err)console.error(err);
                ...
                embeds: [new MessageEmbed().setColor('GREEN').setTitle((Object.keys(player).includes('displayname') ? player.displayname : player.playername) + ' ' + (!player.lastLogin ? '🟢 Online recently' : '🔴 Offline')).setFooter('Last joined Hypixel').setTimestamp(player.lastLogin).addFields({ name: '\u200b', value: 'Currently has ' + player.achievementsOneTime.length + ' achievements.'}, { name: '\u200b', value: player.networkExp.toLocaleString() + ' network xp.'}).setImage('https://crafatar.com/renders/body/' + player.uuid + '?overlay=true')],
