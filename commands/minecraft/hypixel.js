...
module.exports.run = async (client, message, args) => {
    try {
        if (!args.length)message.reply({embeds: [new MessageEmbed().setColor('RED').setDescription('You need to include a player\'s name!')]});
            ...
            embeds: [new MessageEmbed().setColor('GREEN').setTitle(Object.keys(player).includes('displayname') ? player.displayname : player.playername).setFooter('Last joined Hypixel at').setTimestamp(player.lastLogin).addFields({ name: '\u200b', value: 'Currently has ' + player.achievementsOneTime.length + ' achievements.'}, { name: '\u200b', value: player.networkExp.toLocaleString() + ' network xp.'}).setImage('https://crafatar.com/renders/body/' + player.uuid + '?overlay=true')],
                components: [new MessageActionRow().addComponents(new MessageSelectMenu().setCustomId(`hypixel_menu_${message.author.id}_minecraft`).addOptions(Object.keys(player.stats).map(item => ({ label: item, value: item, description: Object.keys(player.stats[item]).includes('games') ? ('played ' + player.stats[item].games.toLocaleString() + ' games') : (Object.keys(player.stats[item]).includes('coins') ? 'has ' + player.stats[item].coins.toLocaleString() + ' coins' : '\u200b')}))))]
            });
        });
    } catch {
